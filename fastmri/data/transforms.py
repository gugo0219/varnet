...
def apply_mask(
    ...
    # empty_mask = np.ones((shape[1], shape[2]))
...
class UnetDataTransform:
    ...
    def __call__(
        ...
        # masked_kspace = masked_kspace[:, 199:595, :]
...
class VarNetDataTransform:
    ...
    def __call__(
        ...
        crop_size = torch.tensor(0)

        if self.mask_func:
            masked_kspace, mask = apply_mask(
                kspace, self.mask_func, mask, seed
            )
        else:
            masked_kspace = kspace
